...
(function(root){
  ...
  root.createPointPicker=function({preview,onConfirm}){
    ...
    let mode="street",countryData=[],busy=false,background=[],tileErrors=new Set();
    const textNode=text=>{const el=document.createElement("span");el.textContent=text;return el;};
    function center(){const p=map.getCenter();return {lat:p.lat,lon:p.lng};}
    ...
    function networkStatus(){
      const active=mode==="satellite"?satellite:map.getZoom()>=STREET_ZOOM?streets:null;
    ...
    function layers(){
      ...
      toggle(streets,mode==="street"&&!broad);toggle(satellite,mode==="satellite");
      countries.setStyle({fillOpacity:mode==="street"&&broad?1:0,weight:broad?.8:0});
      toggle(labels,broad);labels.clearLayers();
      if(broad){
        const bounds=map.getBounds();
        for(const p of countryData){
          if(zoom<p.MIN_LABEL||!bounds.contains([p.LABEL_Y,p.LABEL_X]))continue;
          L.marker([p.LABEL_Y,p.LABEL_X],{interactive:false,keyboard:false,icon:L.divIcon({className:"country-name",html:textNode(p.NAME),iconSize:[110,20],iconAnchor:[55,10]})}).addTo(labels);
        }
      }
    ...
    function init(){
      map=L.map("pointMap",{minZoom:1,maxZoom:22,maxBounds:[[-85,-180],[85,180]],maxBoundsViscosity:1,preferCanvas:true,zoomControl:true,keyboard:true,trackResize:false});
      map.attributionControl.setPrefix(false);L.control.scale({imperial:false}).addTo(map);
      map.createPane("pointCountries").style.zIndex="250";
      map.createPane("offlineLand").style.zIndex="210";
      ...
      streets=L.tileLayer("https://tile.openstreetmap.org/{z}/{x}/{y}.png",{minZoom:STREET_ZOOM,maxNativeZoom:19,maxZoom:22,noWrap:true,attribution:'© <a href="https://www.openstreetmap.org/copyright">OpenStreetMap contributors</a>'});
      satellite=L.tileLayer("https://services.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}",{maxNativeZoom:19,maxZoom:22,noWrap:true,attribution:'Imagery © <a href="https://www.arcgis.com/home/item.html?id=10df2279f9684e4a9f6a7f08febac2a9">Esri, Vantor, Earthstar Geographics, GIS User Community</a>'});
      ...
      fetch("vendor/countries.geojson").then(r=>{if(!r.ok)throw Error();return r.json();}).then(data=>{
        countries.addData(data);countryData=data.features.map(f=>f.properties).filter(p=>Number.isFinite(p.LABEL_X)&&Number.isFinite(p.LABEL_Y));
        for(const extra of [{NAME:"Singapore",LABEL_X:103.82,LABEL_Y:1.35,MIN_LABEL:5},{NAME:"Brunei",LABEL_X:114.75,LABEL_Y:4.5,MIN_LABEL:5}])if(!countryData.some(p=>p.NAME===extra.NAME))countryData.push(extra);
        layers();
      ...
      map.on("click",e=>map.panTo(e.latlng,{animate:false}));
      ...
      const resize=()=>{
        ...
        map.invalidateSize({pan:false,animate:false});map.setView(p,zoom,{animate:false,reset:true});
      };
      if(root.ResizeObserver)new ResizeObserver(resize).observe($("pointMap"));else root.addEventListener("resize",resize);
    }
    function close(){
      map?.stop();overlay.classList.remove("open");cancelAnimationFrame(frame);
    ...
    return {open(opts){
      ...
      map.invalidateSize();map.setView([opts.center.lat,opts.center.lon],opts.zoom||map.getZoom()||12,{animate:false});
      drawPoints(opts.points||[]);layers();update();
      map.invalidateSize({pan:false,animate:false});map.setView([opts.center.lat,opts.center.lon],opts.zoom||map.getZoom(),{animate:false,reset:true});$("pointMap").focus();
    }};
  };
})(globalThis);
