...
**Settings** contains format examples, precision options and your area-of-operations selection. Each preset shows a worked example that follows its own settings, so precision, prefix omission and the chosen reference area all change what you see. A shortened Maps link may need to be opened in a browser first: paste the expanded link containing the coordinates.
