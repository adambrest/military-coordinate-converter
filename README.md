...
**Military grid** (global MGRS / UTM) is disabled by default. Enable it using the button on its Settings row to show it in the converter and the ambiguous-reference chooser. A point outside all supported country presets enables global output automatically; explicit global references also enable it when detected. Disabling it converts resolved global input to latitude/longitude.

Each grid can be enabled or disabled from its collapsed Settings row; disabled grids disappear from the converter and ambiguous-reference chooser. Disabled rows are greyed out. Coordinates always remains enabled.
...
Browser regressions: run `npm install`, `npx playwright install chromium webkit`, then `npm run test:browser`. Use `HEADED=1` to show the mobile emulator windows and `LIVE_IMAGERY=1` to verify live Esri coverage instead of the test fixture. The installed app’s short home-screen label is **Coordinates**.
