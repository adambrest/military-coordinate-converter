...
The **Point Picker** tab is a separate map-first workspace. Move the crosshair and choose **Add center point**, or enable **Tap to add**. Choose global MGRS, coordinates, or a country MGR preset; name your points, copy them, download GPX, or send them to the converter. The collection is saved on this device. Clear and delete can be undone during the current visit.

- **Topo**, **Street** and **Satellite** are selectable on both the Point Picker map and the converter's map picker. Topo is the default. It uses OpenTopoMap (contours and hillshading, CC-BY-SA, SRTM-derived), street uses OpenStreetMap and satellite uses Esri. None of the three needs an account or key, and there is no MapTiler dependency or subscription.
- OpenTopoMap renders to zoom 17; past that the map enlarges its tiles rather than requesting detail that does not exist. Its contours come from SRTM at roughly 30 m, so they show the shape of the ground, not surveyed heights. Its tiles come from volunteer servers with no uptime guarantee.
- The opening view uses an approximate IP/timezone location. No precise geolocation permission is requested.
- At zoom 11 and above, the **1 km grid** shows two-digit kilometer labels with a fading white background at the top (eastings) and left (northings). Country presets use their own projection. Coordinates/global MGRS use the map center's UTM zone and latitude band; the grid is limited to that zone/band and updates when the center changes. It is not a simultaneous multi-zone grid.
...
The current map renderer is 2D. A topographic basemap is now available through OpenTopoMap, but no new contour dataset is bundled and 3D terrain was not added after the MapTiler migration was canceled. Online map providers can still be unavailable; local conversion and saved points remain usable.
...
- A label pasted with its link is kept as the point's name, whether or not there is a space between them (`MLP1 https://…` and `MLP3https://…` both work). Every short link in one paste is followed together, so a list of links costs a single wait and one unreadable line cannot strand the rest. Deleting or editing a row calls off a link it is still following.
